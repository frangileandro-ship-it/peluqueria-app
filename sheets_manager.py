...
def get_sheets_client():
    ...
    try:
        ...
        if not os.path.exists(SERVICE_ACCOUNT_FILE):
            st.error(f"❌ No se encuentra {SERVICE_ACCOUNT_FILE}")
            return None
...
def convertir_fecha_ddmmaaaa(valor):
    ...
    if isinstance(valor, str):
        ...
        # Si falla, intentar con pandas (puede interpretar MM/DD/AAAA)
        try:
            # Intentar con dayfirst=True para forzar DD/MM
...
def load_sheet_data(sheet_name):
    ...
    try:
        ...
        # --- CONVERTIR FECHAS CON FORMATO DD/MM/AAAA ---
